...
.activity.culture {
    background-color: #dbe2e9; /* Bleu pâle */
}
...
.activity.interculturel {
    background-color: #e2e3e5; /* Gris pâle */
}

.activity.intergenerationnel {
...
.activity.complete {
    ...
    background-color: #f8d7da; /* Rose pâle pour signaler la complétude */
}
...
#legend .activity.sport::before {
    color: #d1e7dd;
}

#legend .activity.culture::before {
    color: #dbe2e9;
}

#legend .activity.cuisine::before {
    color: #fff3cd;
}

#legend .activity.interculturel::before {
    color: #e2e3e5;
}

#legend .activity.intergenerationnel::before {
    color: #f8d7da;
}
    </style>
</head>
   ...
   <div class="top-bar">
    ...
    <div class="nav-bar">
        <div class="nav-container">
            ...
            <a href="/appcarefour/planning_brasil.html" class="active">Planning</a>
     ...
     <div class="container">
        <div class="section card" id="planning">
            ...
            <div id="filters">
                ...
                <label><input type="checkbox" class="filter" data-type="culture" checked> Culture</label>
                <label><input type="checkbox" class="filter" data-type="cuisine" checked> Cuisine</label>
                <label><input type="checkbox" class="filter" data-type="interculturel" checked> Interculturel</label>
                <label><input type="checkbox" class="filter" data-type="intergenerationnel" checked> Intergénérationnel</label>
            </div>
            <div id="calendar"></div>
            <div id="legend">
                <h4>Légende :</h4>
                <div class="activity sport">Sport - Événements sportifs</div>
                <div class="activity culture">Culture - Événements culturels</div>
                <div class="activity cuisine">Cuisine - Ateliers de cuisine</div>
                <div class="activity interculturel">Interculturel - Événements interculturels</div>
                <div class="activity intergenerationnel">Intergénérationnel - Activités intergénérationnelles</div>
                <div class="activity complete">Complète - Activités complètes</div>
            </div>
        </div>
    </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const activities = [
        { date: '2024-08-01', type: 'sport', name: 'Match de football', time: '17:00 - 19:00', complete: false },
        { date: '2024-08-02', type: 'interculturel', name: 'Café interculturel', time: '09:00 - 11:00', complete: false },
        { date: '2024-08-05', type: 'culture', name: 'Visite de musée', time: '10:00 - 12:00', complete: false },
        { date: '2024-08-10', type: 'cuisine', name: 'Atelier cuisine italienne', time: '14:00 - 16:00', complete: true },
        { date: '2024-08-15', type: 'intergenerationnel', name: 'Atelier intergénérationnel', time: '13:00 - 15:00', complete: false },
        { date: '2024-08-20', type: 'sport', name: 'Séance de yoga', time: '08:00 - 09:00', complete: false },
        { date: '2024-08-22', type: 'culture', name: 'Soirée théâtre', time: '20:00 - 22:00', complete: false },
        { date: '2024-08-25', type: 'interculturel', name: 'Débat interculturel', time: '16:00 - 18:00', complete: true },
        { date: '2024-08-28', type: 'cuisine', name: 'Atelier pâtisserie française', time: '14:00 - 16:00', complete: false },
        { date: '2024-08-30', type: 'sport', name: 'Tournoi de tennis', time: '10:00 - 13:00', complete: false },
        // Ajouter plus d'activités pour les mois suivants
    ];
    ...
    function renderCalendar(month, year) {
        ...
        daysInMonth.forEach(day => {
            ...
            const activitiesForDay = activities.filter(activity => activity.date === dayDiv.dataset.date);
            activitiesForDay.forEach(activity => {
                if (isActivityVisible(activity)) {
                    const activityDiv = document.createElement('div');
                    activityDiv.className = `activity ${activity.type} ${activity.complete ? 'complete' : ''}`;
                    activityDiv.textContent = `${activity.name} (${activity.time})`;
    ...
    function isActivityVisible(activity) {
        const filterStatus = Array.from(filters).reduce((status, filter) => {
            if (filter.checked) {
                status[filter.dataset.type] = true;
            }
            return status;
        }, {});

        return filterStatus[activity.type] !== false;
    }
    ...
    filters.forEach(filter => {
        filter.addEventListener('change', () => renderCalendar(currentMonth, currentYear));
    });
