...
@media (max-width: 768px) {
 ...
 /* Styles pour la carte et le conteneur */
        ...
        .card #appointments ul {
            list-style-type: none;
            padding: 0;
        }

        .card #appointments ul li {
            background: #f8f9fa;
            padding: 10px;
            border: 1px solid #ddd;
            border-radius: 5px;
        ...
        .card #appointments ul li.no-appointments {
            background: #e9ecef;
            color: #6c757d;
        }
    ...
    <div class="nav-bar">
        <div class="nav-container">
           <a href="/appcarefour/index_brasil.html">À la une</a>
            ...
            <a href="/appcarefour/sante_brasil.html" class="active">Santé</a>
        ...
        <div class="appointment-container">
            ...
            <div class="card">
                ...
                <form id="appointment-form">
                    <label for="category">Catégorie de Professionnel</label>
                    ...
                    <div id="confirmation" class="hidden">Votre rendez-vous a été pris !</div>
                </form>
            </div>
            ...
            <div class="card" id="appointments">
                ...
                <ul>
                    <li class="no-appointments">Aucun rendez-vous à venir</li>
                    <!-- Les rendez-vous seront ajoutés ici dynamiquement -->
                </ul>
            </div>

            <!-- Section Carte des Professionnels de Santé -->
            <div class="map-container">
                <h2>Trouver un professionnel de santé à proximité de nos locaux</h2>
                <div id="map"></div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const professionals = {
        generaliste: [
            { name: 'Dr. Ana Silva', lat: -22.9068, lng: -43.1729, hours: { '2024-07-25': ['09:00', '10:00', '11:00'] } }, // Rio de Janeiro
            { name: 'Dr. Carlos Oliveira', lat: -23.5505, lng: -46.6333, hours: { '2024-07-25': ['14:00', '15:00', '16:00'] } }  // São Paulo
        ],
        dentiste: [
            { name: 'Dr. Beatriz Santos', lat: -3.7172, lng: -38.5433, hours: { '2024-07-26': ['09:00', '10:00', '11:00'] } } // Fortaleza
        ],
        cardiologue: [
            { name: 'Dr. João Costa', lat: -12.9714, lng: -38.5014, hours: { '2024-07-27': ['11:00', '13:00', '15:00'] } } // Salvador
        ]
    };
    ...
    document.getElementById('appointment-form').addEventListener('submit', (event) => {
        ...
        if (selectedProfessional && selectedProfessional.hours[date] && selectedProfessional.hours[date].includes(time)) {
            document.getElementById('confirmation').classList.remove('hidden');
            const appointmentsList = document.querySelector('#appointments ul');

            if (appointmentsList.querySelector('.no-appointments')) {
                appointmentsList.innerHTML = ''; // Enlever le message "aucun rendez-vous"
            }

            const listItem = document.createElement('li');
            listItem.textContent = `Catégorie: ${category}, Date: ${date}, Heure: ${time}`;
            appointmentsList.appendChild(listItem);
        } else {
            alert('Choisissez une date et une heure disponibles pour le professionnel sélectionné.');
        }
    });

    // Initialiser la carte
    const map = L.map('map').setView([-14.2350, -51.9253], 4); // Vue centrée sur le Brésil
    ...
    Object.values(professionals).flat().forEach(professional => {
        L.marker([professional.lat, professional.lng]).addTo(map)
            .bindPopup(`<b>${professional.name}</b><br>Disponibilités: ${Object.keys(professional.hours).join(', ')}`)
            .on('click', () => {
                selectedProfessional = professional;
                alert(`Vous avez sélectionné ${professional.name}. Veuillez choisir une date et une heure.`);
            });
    });
});
    </script>
